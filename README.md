...
By now you probably have noticed how slow using Conda is on the cluster. – I’m uncertain if this is correct since the machines in our cluster have high network bandwidth, ample storage and plenty of compute power. But I suspect that NFS4 might be sensitive to handling a large number of files, like those generated by Conda. This could explain why some file-related commands take up to an hour to run, while heavy computational tasks using CPU/GPU resources complete quickly.
