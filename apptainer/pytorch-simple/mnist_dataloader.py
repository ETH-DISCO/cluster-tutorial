...
class MnistDataloader(object):
    ...
    def read_images_labels(self, images_filepath, labels_filepath):
        ...
        images = np.stack([np.array(image_data[i * rows * cols : (i + 1) * rows * cols]).reshape(28, 28) for i in range(size)])
        # for i in range(size):
...
def get_dataloader(data_path: str | None = None, batch_size: int = 32, shuffle: bool = True) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    ...
    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(torch.tensor(x_train, dtype=torch.float32).unsqueeze(1), torch.tensor(y_train, dtype=torch.int64)), batch_size=batch_size, shuffle=shuffle)
    test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(torch.tensor(x_test, dtype=torch.float32).unsqueeze(1), torch.tensor(y_test, dtype=torch.int64)), batch_size=batch_size, shuffle=shuffle)
